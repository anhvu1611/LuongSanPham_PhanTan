...
import form.PanelChamCongCongNhan;
import form.PanelChamCongNhanVien;
import form.PanelPhanCongCongDoan;
...
public class GUI_APP extends JFrame {
	...
	private MenuItem menuHomePage, menuSanPham, menuCapNhatSP, menuPhanCong, menuTimKiemSP, menuTaoCongDoan,
			menuNhanVien, menuCapNhatNV, menuChamCongNV, menuLuongNV, menuTimKiemNV, menuCongNhan, menuCapNhatCN,
			menuChamCongCN, menuLuongCN, menuTimKiemCN, menuThongKe, menuThongKeLuongNV, menuThongKeLuongCN,
			menuThongKeSP;
	...
	public GUI_APP(NhanVien nv, DataOutputStream out, ObjectInputStream in, Socket socket) {
		this.outData = out;
		this.in = in;
		this.socket = socket;
		initComponents(nv);
	...
	private void execute(NhanVien nv) {
		...
		if (nv.getChucVu().getMaChucVu().equalsIgnoreCase("CV0005")) {
			...
			menuTimKiemSP = new MenuItem(iconBill2, "Tìm kiếm", new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					panelBody.removeAll();
					panelBody.add(new frm_SanPham_TimKiem(outData, in, socket));
					panelBody.repaint();
					panelBody.revalidate();
				}
			});
...
//            		  menuCapNhatSP, menuPhanCong,menuTimKiemSP, menuTaoCongDoan
			...
			menuTimKiemNV = new MenuItem(iconBill2, "Tìm kiếm", new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					panelBody.removeAll();
					panelBody.add(new frm_NhanVien_TimKiem(outData, in, socket));
					panelBody.repaint();
					panelBody.revalidate();
				}
			});
...
//            		  menuCapNhatNV, menuChamCongNV,menuLuongNV,menuTimKiemNV
			...
			menuLuongCN = new MenuItem(iconBill2, "Lương", new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					panelBody.removeAll();
					panelBody.add(new frm_CongNhan_TinhLuong(outData, in, socket));
					panelBody.repaint();
					panelBody.revalidate();
				}
			});
...
//            		  menuCapNhatCN, menuChamCongCN,menuLuongCN,menuTimKiemCN
			...
			menuThongKeLuongNV = new MenuItem(iconBill1, "Lương nhân viên", new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					panelBody.removeAll();
					panelBody.add(new Frm_ThongKe_NhanVien(outData, in, socket));
					panelBody.repaint();
					panelBody.revalidate();
				}
			});
			...
			menuCapNhatNV = new MenuItem(iconBill1, "Cập nhật", new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					panelBody.removeAll();
					try {
						panelBody.add(new frm_NhanVien_CapNhat(outData, in, socket));
						panelBody.repaint();
						panelBody.revalidate();
					} catch (Exception e1) {
			...
			menuCapNhatCN = new MenuItem(iconBill1, "Cập nhật", new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					panelBody.removeAll();
					panelBody.add(new frm_CongNhan_CapNhat(outData, in, socket));
					panelBody.repaint();
					panelBody.revalidate();
				}
			});
			...
			menuCapNhatSP = new MenuItem(iconBill1, "Cập nhật", new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					panelBody.removeAll();
					try {
						panelBody.add(new frm_SanPham_CapNhat(outData, in, socket));
			...
			menuChamCongNV = new MenuItem(iconBill2, "Chấm công", new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					panelBody.removeAll();
					panelBody.add(new PanelChamCongNhanVien(nv));
					panelBody.repaint();
					panelBody.revalidate();
				}
			});
			...
			menuChamCongCN = new MenuItem(iconBill2, "Chấm công", new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					panelBody.removeAll();
					panelBody.add(new PanelChamCongCongNhan(nv));
					panelBody.repaint();
					panelBody.revalidate();
				}
			});
...
//                      menuHomePage.setBackground(menus.getBackground());
	...
	// <editor-fold defaultstate="colln apsed" desc="Generated
	...
	private void initComponents(NhanVien nv) {
		...
		jLabel3.setIcon(new ImageIcon(getClass().getResource("/item/wood-80.png"))); // NOI18N
				...
				.setHorizontalGroup(
						panelHeaderLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
								.addGroup(panelHeaderLayout.createSequentialGroup().addGap(20, 20, 20)
										...
										.addComponent(jLabel4).addGap(146, 146, 146).addComponent(jLabel8,
												javax.swing.GroupLayout.DEFAULT_SIZE, 939, Short.MAX_VALUE)
										.addGap(54, 54, 54)));
		panelHeaderLayout.setVerticalGroup(panelHeaderLayout
				.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(panelHeaderLayout.createSequentialGroup().addGap(13, 13, 13).addGroup(panelHeaderLayout
		...
		jLabel1.setIcon(new ImageIcon(getClass().getResource("/item/nhanVien.png"))); // NOI18N
		...
		btnChangePass.setIcon(new ImageIcon(getClass().getResource("/item/key.png"))); // NOI18N
		...
		btnLogout.setIcon(new ImageIcon(getClass().getResource("/item/doiPass.png"))); // NOI18N
	...
	protected void thoatKhoiChuongTrinh() {
		int lc = JOptionPane.showConfirmDialog(null, "Bạn chắc chắn muốn thoát khỏi chương trình", "Chú ý",
	...
	private void btnChangePassActionPerformed(ActionEvent evt, TaiKhoan tk) {// GEN-FIRST:event_btnChangePassActionPerformed
		Dialog_DoiMatKhau dialog_DoiMatKhau = new Dialog_DoiMatKhau(new Frame(), true, tk, outData, in, socket);
