...
public class frm_CongNhan_CapNhat extends javax.swing.JPanel {
    ...
    public frm_CongNhan_CapNhat(DataOutputStream out, ObjectInputStream in, Socket socket) {
    	this.outData = out;
		this.in = in;
		this.socket = socket;
        initComponents();
    	...
    	try {
			if(tglThem.isSelected()) {
		        ...
		        if(isSuccess) {
			        ...
			        thietLapLaiForm();
			        dongFormNhapDuLieu();	
			        JOptionPane.showMessageDialog(null, "Thêm công nhân thành công");
		        }else {
			        JOptionPane.showMessageDialog(null, "Thêm công nhân thất bại");	
		        }
...
//        		qlDiaChi.taoDiaChiVaTraDiaChi(diaChiCuThe, thanhPho_Tinh, quanHuyen, thiXa);
        ChuyenMon chuyenMon = (ChuyenMon) cmbChuyenMon.getSelectedItem();
        CongNhan cn = new CongNhan(ma, ho, ten, sdt, ngaySinh, ngayVaoLam, cccd, diaChi, PhuCap.MUC_2, gioiTinh.getGioiTinh(), anhNguoiDung, false, chuyenMon, CaLamCongNhan.CAMOT);
        return cn;
    }
...
//            		qlDiaChi.layQuanHuyenDuaTheoTinhThanhPho(TinhThanhPhpDaChon.trim());
...
//            		qlDiaChi.layPhuongXaDuaTrenQuanHuyenVaTinhThanhPho(TinhThanhPhpDaChon.trim(), quanHuyenDaChon.trim());
	...
	private void loadDuLieuChuyenMon(){
    	try {
    		outData.writeUTF("CHUYENMON");
        	outData.writeInt(1);
        	outData.flush();
            cmbChuyenMon.removeAll();
            List<ChuyenMon> ds = (List<ChuyenMon>) in.readObject();
//            		qlChuyenMon.getDs()
...
//            		qlCongNhan.getDs().toArray(new CongNhan[0]);
    ...
    private void loadDuLieuCongNhanRaBang(DefaultTableModel modal) {
    	try {
            ...
            outData.flush();
        	@SuppressWarnings("unchecked")
			List<CongNhan> dsCongNhan = (List<CongNhan>) in.readObject();
//        			qlCongNhan.getDs();
...
//    }
    ...
    private CongNhan layDuLieuCongNhanTheoMaTuCot(int dongDuocChon) {
		try {
			...
			outData.writeUTF("CONGNHAN");
			outData.writeInt(1);
			outData.writeUTF(maCongNhan);
			outData.flush();
			return (CongNhan) in.readObject();
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
