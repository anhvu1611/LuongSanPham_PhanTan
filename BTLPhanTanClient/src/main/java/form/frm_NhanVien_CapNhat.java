...
public class frm_NhanVien_CapNhat extends javax.swing.JPanel implements Serializable {
    ...
    public frm_NhanVien_CapNhat(DataOutputStream out, ObjectInputStream in, Socket socket) {
    ...
    private void layMaNhanVien() {
    	try {
    		outData.writeUTF("GD_NHANVIEN_CN");
        	outData.writeInt(7);
        	outData.flush();
            String maNhanVien = in.readUTF();
            System.out.println(maNhanVien);
//            		NhanVien_Dao.layMaNhanVienTiepTheo();
    ...
    private void bangDuLieuNhanVien(){
        ...
        modal = new javax.swing.table.DefaultTableModel(column, 50) {
			 ...
			 * 
			 */
			private static final long serialVersionUID = 1419479062427729619L;
	...
	protected void capNhatDuLieuThiXa() {
		try {
			System.out.println("Vào thị xã");
			String TinhThanhPhpDaChon = (String) cmbThanhPho.getSelectedItem();
			String quanHuyenDaChon = (String) cmbQuanHuyen.getSelectedItem();
			if(TinhThanhPhpDaChon == null || quanHuyenDaChon == null) {
				return;
			}
	    	outData.writeUTF("GD_NHANVIEN_CN");
	    	outData.writeInt(3);
	    	outData.flush();
	    	outData.writeUTF(TinhThanhPhpDaChon);
	    	outData.writeUTF(quanHuyenDaChon);
	    	
			String thiXa[] = (String[]) in.readObject();
//					qlDiaChi.layPhuongXaDuaTrenQuanHuyenVaTinhThanhPho(TinhThanhPhpDaChon.trim(), quanHuyenDaChon.trim());
	...
	protected void capNhatDuLieuQuanHuyen() {
		try {
	    	...
	    	outData.writeUTF(TinhThanhPhpDaChon);
	    	outData.flush();
			String quanHuyen[] = (String[]) in.readObject();
			for(int i=0;i<quanHuyen.length;i++)
				System.out.println(quanHuyen[i]);
	...
	private void datDuLieuNhanVienVaoForm(int dongDuocChon) {
		...
		//bhyt
		if (nv.getBHXH().equalsIgnoreCase("Tham Gia")) {
		    chkBHXH.setSelected(true);
		} else {
		    chkBHXH.setSelected(false);
		}
		//bhxh
		if (nv.getBHYT().equalsIgnoreCase("Tham Gia")) {
    ...
    private void btnXacNhanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnXacNhanActionPerformed
        ...
        try {
			if(tglThem.isSelected()) {
		        ...
		        outData.writeUTF(json);
		        outData.flush();
		        boolean isSuccess = (boolean) in.readObject();
		        if(isSuccess) {
			        thietLapLaiForm();
			        dongFormNhapDuLieu();
			        loadDuLieuNhanVienRaBang(modal);
		            ...
		            dongFormNhapDuLieu();		        
			        tglThem.setSelected(false);
			        JOptionPane.showMessageDialog(null, "Thêm nhân viên thành công");
		        }else {
		        	JOptionPane.showMessageDialog(null, "Thêm nhân viên thất bại");
		        }
//		        
		        ...
		        if(isSuccess) {
			        ...
			        tglThem.setSelected(false);
			        tblSua.setSelected(false);
			        JOptionPane.showMessageDialog(null, "Sửa nhân viên thành công");
		        }else {
    ...
    private void loadDuLieuNhanVienRaBang(DefaultTableModel modal) throws Exception {
    	int i=1;
    	...
    	outData.writeInt(4);
    	outData.flush();
    	List<NhanVien> ds = (List<NhanVien>) in.readObject();
//    	qlNhanVien.getDs();s
        for (NhanVien nv : ds) {
    ...
    private PhongBan[] danhSachPhongBan(){
    	try {
    		outData.writeUTF("GD_NHANVIEN_CN");
        	outData.writeInt(6);
        	List<PhongBan> dsPhongBan = (List<PhongBan>) in.readObject();  
//            ArrayList<PhongBan> dsPhongBan = qlPhongBan.getDs();
    ...
    private ChucVu[] danhSachChucVu(){
    	try {
    		outData.writeUTF("GD_NHANVIEN_CN");
        	outData.writeInt(5);
        	List<ChucVu> dsPhongBan = (List<ChucVu>) in.readObject();  
//            ArrayList<PhongBan> dsPhongBan = qlPhongBan.getDs();
        	for (ChucVu phongBan : dsPhongBan) {
				System.out.println(phongBan.toString());
			}
            ChucVu[] mang = new ChucVu[dsPhongBan.size()];
            mang = dsPhongBan.toArray(mang);
            return mang;
		} catch (Exception e) {
			// TODO: handle exception
    ...
    private NhanVien taoNhanVien() {
    	try {
            ...
            DiaChi diaChi = new DiaChi(diaChiCuThe, thiXa, quanHuyen, thanhPho_Tinh);
            ...
            NhanVien nv = 
            		new NhanVien(ma, ho, ten, sdt, ngaySinh, ngayVaoLam, cccd, diaChi, phuCap, gioiTinh.getGioiTinh(), anhNguoiDung, chucVu, null, bhyt, bhxh, phongBan, email, heSoLuong, thamNien);
    ...
    private void caiDuLieuDiaChi() throws IOException, ClassNotFoundException {
    	outData.writeUTF("GD_NHANVIEN_CN");
    	outData.writeInt(1);
    	outData.flush();
    	String thanhPho[] = (String[]) in.readObject();
//    			qlDiaChi.layDanhSachTinhThanhPho();
    	for(int i = 0;i< thanhPho.length;i++) {
    		System.out.println(thanhPho[i]);
    	}
